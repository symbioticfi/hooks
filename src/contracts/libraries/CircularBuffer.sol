...
/// @notice A fixed-size buffer for keeping `bytes32` items in storage.
/// @author Modified from OpenZeppelin (https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/utils/structs/CircularBuffer.sol)
library CircularBuffer {
    ...
    function setup(Bytes32CircularBuffer storage self, uint256 size) internal {
        ...
        clear(self);
        bytes32[] storage array = self._data;
        assembly ("memory-safe") {
            sstore(array.slot, size)
        }
    }
